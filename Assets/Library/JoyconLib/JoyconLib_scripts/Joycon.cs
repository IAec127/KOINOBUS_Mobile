...
public class Joycon
{
	...
	// ノイズによるブレはだいたい±12、ただ日によって気まぐれにズレるので再補正回避で2倍くらいに
	private UInt16 gyr_threshold = 2;
    ...
    private void ExtractIMUValues(byte[] report_buf, int n = 0)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
			...
			// フィルター
			if (-1 * gyr_threshold < (gyr_r[i] - gyr_neutral[i]) && (gyr_r[i] - gyr_neutral[i]) < gyr_threshold)
			{
				gyr_g[i] = 0f;
			}
			else
